...
export async function getHomeContent() {
  const resp = await fetch(`${BASE}/api/public/home`, { headers: { ...authHeader() } });
  if (!resp.ok) throw new Error("Gagal mengambil konten beranda");
  return resp.json();
}

export async function getAboutContent() {
  const resp = await fetch(`${BASE}/api/public/about`, { headers: { ...authHeader() } });
  if (!resp.ok) throw new Error("Gagal mengambil konten tentang kami");
  return resp.json();
}

export async function getSiteSettings() {
  const resp = await fetch(`${BASE}/api/admin/site-settings`, { headers: { ...authHeader() } });
...
export async function upsertSiteContent(payload: { id?: string; page: "home" | "about"; section: string; data: unknown; status?: "PUBLISHED" | "DRAFT"; sort_order?: number }) {
...
export async function listSiteContents(params?: { page?: "home" | "about"; section?: string; published?: boolean }) {
  const q = new URLSearchParams();
  if (params?.page) q.set("page", params.page);
  if (params?.section) q.set("section", params.section);
  if (typeof params?.published === "boolean") q.set("published", String(params.published));
  const resp = await fetch(`${BASE}/api/admin/site-contents${q.toString() ? `?${q.toString()}` : ""}`, { headers: { ...authHeader() } });
...
export async function deleteSiteContent(id: string, section?: string) {
  ...
  const resp = await fetch(`${BASE}/api/admin/site-contents/${id}${q}`, { method: "DELETE", headers: { ...authHeader() } });
