            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-4">
                ...
                <SearchableSelect
                  ...
                  options={[{ value: "", label: "Semua Tipe" }, ...types]}
                  value={type}
                  onChange={setType}
                />
                <SearchableSelect
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <p className="text-sm text-gray-500">
                  Menampilkan {filtered.length} dari {jobs.length} lowongan
                </p>
              </div>
  ...
  const card = (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-3 sm:p-4 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3 hover:shadow-md transition-shadow cursor-pointer overflow-hidden">
      ...
      <div className="flex flex-col items-start sm:items-end gap-2 shrink-0 mt-1 sm:mt-0 w-full sm:w-auto">
        ...
        <span
          className={`px-2 py-1 text-[11px] sm:text-xs rounded-full ${closed ? "bg-red-50 text-red-600" : "bg-gray-100 text-gray-500"}`}
        >
          {closed ? "Tutup" : "Tutup pada"} {dateLabel}
        </span>
      </div>
    </div>
  );
      ...
      onClick={() => {
        try {
          if (typeof window !== "undefined") {
            sessionStorage.setItem("last_job", JSON.stringify(job));
          }
        } catch {}
      }}
      className="block"
